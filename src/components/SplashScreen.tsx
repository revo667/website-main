...
export function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[60] flex items-center justify-center bg-gradient-to-b from-neutral-950 via-neutral-900 to-neutral-950 transition-all duration-1000 ease-in-out ${leaving ? "pointer-events-none scale-105 opacity-0" : "scale-100 opacity-100"}`}
    >
      {}
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-4">
        ...
        {}
        <div
          className={`w-full max-w-sm rounded-lg border border-violet-500/30 bg-neutral-900/80 p-6 shadow-[0_0_40px_rgba(139,92,246,0.3)] backdrop-blur-md transition-all duration-500 ${clicked ? "hidden" : "animate-in fade-in slide-in-from-bottom-4"}`}
        >
            ...
            {SOUNDS.map((sound) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded border-l-4 px-3 py-2 text-left text-xs uppercase tracking-wider transition-all duration-200 ${selected ? "border-violet-500 bg-violet-500/20 font-semibold text-violet-300" : "border-white/10 bg-white/5 text-white/50 hover:bg-white/10 hover:text-white/80"}`}
                >
